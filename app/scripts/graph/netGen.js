// // This file is yet to be refactored and i doubt if it should be refactored as
// // we will probably want to manually generate the network
...
// // json = {
// //     "nodes":[{"name":"a","group":1}, {"name":"b","group":2}, {"name":"c","group":3}],
// //     "links":[{"source":1,"target":0,"value":1}, {"source":2,"target":0,"value":2}]
// // };
...
// var connect = function(v, to, reverse) {
// 	var toRet = [];
// 	for (var i=0; i<to.length; i++) {
// 		if (reverse) {
// 			toRet.push(getE(to[i], v));
// 		} else {
// 			toRet.push(getE(v, to[i]));			
// 		}
// 	}
// 	return toRet;
// };

// var connectRule = function(v, starter, increment) {
// 	var toRet = [];
// 	for (var i=0; i<to.length; i++) {
// 		toRet.push(getE(v, to[i]));
// 	}
...
// var getEdges = function(edges, vertice, arr, reverse) {
...
// 		var t = i*step + min;
// 		var arr = [(t), (1+t), (2+t)];
// 		edges = getEdges(edges, someLayer[i], arr);
// 		console.log('t:' +t + ', ' + arr);
// 	}	
// 	return edges;
// };
// /* Main */
// var secondLayer = [2,3,4];
// var thirdLayer = [5, 6, 7, 8, 9, 10, 11];
// var forthLayer = [12,13,14,15,16];
// var fifthLayer = [17,18];
// var sixthLayer = [19,20];
// var vertices = [1].concat(secondLayer).concat(thirdLayer).concat(forthLayer).concat(fifthLayer);
// //First Layer
// var edges = connect(1, [2,3,4]);

// // Second Layer
// edges = connectPattern(edges, secondLayer, thirdLayer[0], 2);

// // Third Layer
// var forthLen = forthLayer.length;
// var thirdLen = thirdLayer.length;

// var t = 1;
// edges = getEdges(edges, thirdLayer[t-1], forthLayer.slice(0, t));
// edges = getEdges(edges, thirdLayer[thirdLen-t], forthLayer.slice(forthLen-t, forthLen));
// t++;
// edges = getEdges(edges, thirdLayer[t-1], forthLayer.slice(0, t));
// edges = getEdges(edges, thirdLayer[thirdLen-t], forthLayer.slice(forthLen-t, forthLen));

// edges = connectPattern(edges, thirdLayer.slice(t, thirdLen-t), forthLayer[0], 1);

// // Forth Layer
// t = 3;
// edges = getEdges(edges, fifthLayer[0], forthLayer.slice(0, t), true);
// edges = getEdges(edges, fifthLayer[1], forthLayer.slice(forthLayer.length-t, forthLayer.length), true);
...
// 	return _.filter(edges, function(e){return (e.source.group==vId);});
// }


// function getRandomVertice() {
// 	return 	vertices[getRandomArbitrary(0, vertices.length-1)];
// }
// function getRandomEdge() {
// 	return 	edges[getRandomArbitrary(0, edges.length-1)];
// }
